...
React是一个构建用户界面的js库, 围绕者React有很丰富的生态。通常React和react-router, redux, webpack, es6等结合起来开发应用。React具有如下特点:

- 描述式的

    React使创建交互界面变得轻松。React是数据驱动的视图，每次(状态)的数据的改变, React都会高效的更新和渲染每个组件。这种数据驱动的方式使代码可预测，容易理解和调试。
